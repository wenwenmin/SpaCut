...
def get_gene_map(args):
    ...
    df_transcripts = df_transcripts[~df_transcripts["gene"].str.startswith("Blank")] # Remove blank genes
    ...
    '''
    HumanOvarianCancerPatient2Slice2_images_micron_to_mosaic_pixel_transform.csv
    9.259366035461425781e+00 0.000000000000000000e+00 -4.368514453125000000e+04
    0.000000000000000000e+00 9.259420394897460938e+00 -5.156664843750000000e+04
    0.000000000000000000e+00 0.000000000000000000e+00 1.000000000000000000e+00
    第一行的第一个数表示 x 方向的放大(缩放)因子，第一行第三个数表示 x 方向的平移量（负值表示向左平移）
    第二行的第二个数表示 y 方向的放大(缩放)因子，第二行第三个数表示 y 方向的平移量（负值表示向上平移）
    '''
    ...
    n_processes = mp.cpu_count() / 2 # 除2减小cpu的压力
    gene_names_chunks = np.array_split(gene_names, n_processes)
    processes = []
    # Create temp dir
    # temp_dir = tempfile.mkdtemp()
    temp_dir = tempfile.mkdtemp(dir="D:/paper/newmodel/temp_dir")
...
def get_vigzen_polygon(fov, cell_boundary_h5_path, cell_meta_path, transform_mat_path, z_index=2):
    cellBoundaries = h5py.File(cell_boundary_h5_path) # h5 文件与 cell_metadata.csv是一一对应的
...
def prepare_dapi_for_cellpose(args): 
    ...
    for z in z_list[1:]:
        ...
        dapi_0 = np.maximum(dapi_0, dapi) # max projection，保留像素值最大的那个值
    # Scale
    dapi = dapi_0
    transformation_matrix = pd.read_csv(args.transform_matrix, header=None, sep=' ').values
    ...
    gene_map = tifffile.imread(os.path.join(args.out_dir, "gene_map.tif"))  # resize

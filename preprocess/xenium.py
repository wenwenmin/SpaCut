...
def get_gene_map(args):
    ...
    print("Converting to expression maps")
    n_workers = min(mp.cpu_count() * 8 ,20) #   线程数设计
    gene_names_chunks = np.array_split(gene_names, n_workers)
    
    # 使用线程
    with ThreadPoolExecutor(max_workers=n_workers) as executor:
        futures = [executor.submit(process_gene_chunk, chunk, df, 
                                  map_height, map_width, temp_dir) 
                  for chunk in gene_names_chunks]
        
        # 创建更有意义的进度条 - 显示实际处理的基因数
        completed = 0
        with tqdm(total=len(gene_names), desc="Genes processed") as pbar:
            for future in futures:
                chunk_result = future.result()  # 等待完成
                # 更新已处理的基因数量
                genes_in_chunk = sum(len(chunk) for chunk in gene_names_chunks if future in futures)
                pbar.update(genes_in_chunk)
...
def get_10X_segmentation(args):
    ...
    print("Processing cell boundaries、nucleus boundaries......")
    for m, id in bar:
        mask_id = m + 1

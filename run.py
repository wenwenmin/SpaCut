...
from src.newmodel import SpaCutModel
...
if __name__ == "__main__":
    ...
    train_dataset = MyDataset(manager)
    
    train_dataset.set_train(True)   
    gene_map_shape = train_dataset.gene_map.shape

    # Initializing the model
    model = SpaCutModel(manager, gene_map_shape)
    model.train_model(train_dataset)

    print("Initializing pred dataset with shift 0 and shift patch size//2")
    
    pred_dataset_0 = MyDataset(manager, 0)
    pred_dataset_1 = MyDataset(manager, opt.patch_size//2)

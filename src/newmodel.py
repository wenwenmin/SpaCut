...
import shutil
import threading
from src.newmodel_utils import ForegroundPredictNet, CellPredictNet, get_softmask, get_seg_mask

class SpaCutModel(nn.Module):
    def __init__(self, manager, gene_map_shape):
        super(SpaCutModel, self).__init__()
    ...
    def postprocess(self):
        ...
        # 使用线程代替多进程
        num_threads = min(os.cpu_count() * 2, 20)  # 不超过20个线程
        self.logger.info('Num threading splits: %d' % num_threads)
        coords_splits = np.array_split(coords_starts, num_threads)
        threads = []

        output_dir = os.path.join(self.pred_dir, "temp_dir_{}".format(int(time.time())))
        os.makedirs(output_dir, exist_ok=True)


        for chunk in coords_splits:
            t = threading.Thread(target=self.process_chunk, args=(chunk, patch_size, img_whole, nuclei_img, output_dir))
            threads.append(t)
            t.start()
        for t in threads:
            t.join()
        ...
        ids_splits = np.array_split(to_check_ids, num_threads)
        threads = []
        for chunk_ids in ids_splits:
            t = threading.Thread(target=self.process_check_splits, args=(nuclei_img, seg_final, chunk_ids))
            threads.append(t)
            t.start()
        for t in threads:
            t.join()
    ...
    def process_chunk(self, chunk, patch_size, img_whole, nuclei_img, output_dir):
        ...
        for index in range(len(chunk)):
            ...
            output_fp = os.path.join(output_dir, '%d_%d.tif' % (coords_x1, coords_y1))

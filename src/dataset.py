...
class MyDataset(data.Dataset):
    def __init__(self, manager, shift_patches=0):
        ...
        self.logger.info(f"Gene map shape: {self.gene_map.shape}") # (h, w, n_gene) 
    ...
    def __getitem__(self, index):
        ...
        img = np.concatenate((expr, np.expand_dims(nucl, -1)), -1) # nucl (patch_size, patch_size, 1) -> (patch_size, patch_size, 1 + n_gene)
...
# expr_torch：（batch_size,patch_size,patch_size,n_gene）
# nucl_torch：（batch_size,patch_size,patch_size）
# mask_torch：（batch_size,patch_size,patch_size）
# bg_mask_torch：（batch_size,patch_size,patch_size）